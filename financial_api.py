...
def analyze_stock(symbol):
    analyzer = FinancialAnalyzer()
    return analyzer.basic_company_analysis(symbol)

def analyze_portfolio(symbols):
    pm = PortfolioManager(symbols)
    return pm.portfolio_overview()

def recommend_short_term_investment(amount_gbp, months):
    if months <= 3:
        return f"For {months} months, consider a high-interest savings account or fixed deposit."
    elif months <= 12:
        return f"Consider short-term bonds or diversified ETFs."
    else:
        return f"Look into balanced portfolios or index funds."
